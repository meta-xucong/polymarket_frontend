...
def build_run_config_from_mapping(raw: Dict[str, Any]) -> RunConfig:
    """Build a :class:`RunConfig` from a loose mapping.

    - 允许以 ``side`` 或 ``direction`` 传入方向，自动转大写。
    - 允许使用百分数（5 表示 5%）或小数（0.05 表示 5%）作为跌幅/盈利阈值。
    - 允许通过 ``market_source`` 或 ``source`` 传入市场链接/slug。
    """
    ...
    return RunConfig(
        source=str(_first("market_source", "source", default="")).strip(),
        direction=str(_first("direction", "side", default="")).upper(),
        sell_mode=_first("sell_mode", default="aggressive") or "aggressive",
        size=_float(_first("size")),
        buy_price_threshold=_float(_first("buy_price_threshold", "buy_price")),
        drop_window_minutes=_float(_first("drop_window_minutes", "drop_window"), 10.0)
        or 10.0,
        drop_pct=_pct(_first("drop_pct", "drop_percentage"), 0.05) or 0.05,
        profit_pct=_pct(_first("profit_pct", "profit_percentage"), 0.05) or 0.05,
        enable_incremental_drop_pct=bool(
            _first("enable_incremental_drop_pct", "incremental", default=True)
        ),
        countdown_absolute_ts=_float(
            _first("countdown_absolute_ts", "sell_only_start_ts")
        ),
        countdown_minutes_before=_float(_first("countdown_minutes_before")),
        yes_token_id=_first("yes_token_id"),
        no_token_id=_first("no_token_id"),
        market_meta=_first("market_meta", default={}),
        market_deadline_ts=_float(_first("market_deadline_ts")),
        title=_first("title"),
    )
