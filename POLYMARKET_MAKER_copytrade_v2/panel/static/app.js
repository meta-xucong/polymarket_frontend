...
function updateV3ProfileLink() {
  ...
  updateLinkState(
    document.getElementById("v3-profile-link"),
    address,
    "暂无可跳转地址"
  );
}
...
function buildV2RuntimeSummary(payload) {
  ...
  const items = [
    ["活跃 Token 数", payload.active_token_count ?? 0],
    ["Copytrade 状态", services.copytrade?.raw || "unknown"],
    ["Copytrade PID", services.copytrade?.pid ?? "-"],
    ["Autorun 状态", services.autorun?.raw || "unknown"],
    ["Autorun PID", services.autorun?.pid ?? "-"],
    ["最近更新", payload.copytrade_updated_at || "-"],
  ];

  return items
    .map(
      ([label, value]) =>
        `<div class="stat"><strong>${label}</strong><span>${value}</span></div>`
    )
    .join("");
}
...
function buildV3RuntimeSummary(payload) {
  ...
  const items = [
    ["运行状态", service.raw || "unknown"],
    ["进程 PID", service.pid ?? "-"],
    ["启用账户数", payload.active_account_count ?? 0],
    ["跟单目标数", payload.target_address_count ?? 0],
    ["日志文件", payload.log_file || "-"],
  ];
...
async function refreshRuntime(preferredLog = null) {
  ...
  const mode = payload.services?.mode || "";
  pill.textContent = mode === "local-process" ? "本地进程控制" : "systemctl 控制";

  const viewer = document.getElementById("log-viewer");
  viewer.textContent =
    preferredLog === "copytrade"
      ? payload.copytrade_log_tail || "暂无日志"
      : preferredLog === "autorun"
        ? payload.autorun_log_tail || "暂无日志"
        : payload.autorun_log_tail || payload.copytrade_log_tail || "暂无日志";
}
...
async function refreshV3Runtime() {
  ...
  const pill = document.getElementById("v3-status-pill");
  const mode = payload.services?.mode || "";
  pill.textContent = mode === "local-process" ? "本地进程控制" : "systemctl 控制";

  document.getElementById("v3-log-viewer").textContent =
    payload.copytrade_log_tail || "暂无日志";
}
...
async function deleteV3Account() {
  ...
  if (!index) {
    throw new Error("当前没有可删除的账户");
  }
  if (!window.confirm("确认删除当前账户吗？此操作会直接从 accounts.json 中移除该账户。")) {
    return;
  }
...
async function handleServiceAction(event) {
  ...
  button.textContent =
    action === "stop" ? "停止中..." : action === "start" ? "启动中..." : "重启中...";
  try {
    const payload = await readJson(`/api/service?name=${service}&action=${action}`, {
      method: "POST",
    });
    showToast(payload.ok ? `${service} ${action} 完成` : payload.message, !payload.ok);
